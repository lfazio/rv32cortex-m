...
/* ------------------------------------------------------------------ */
/* Scratch-register residency                                          */
/* ------------------------------------------------------------------ */

/*
 * Which guest register R1 is known to hold, or -1.
 *
 * The register file lives in memory, so a guest instruction ends by
 * storing its result and the next one begins by loading its operand. When
 * the two are the same register -- 24% to 33% of adjacent executed pairs,
 * measured with RV32_PAIR_STATS -- the load reads back what the store just
 * wrote and can simply not be emitted.
 *
 * This is deliberately *not* the r8-r10 register cache that measured 15.5%
 * slower. That one pre-loaded registers at block entry and wrote through,
 * paying a setup load per register and three more callee-saved registers on
 * every PUSH/POP; a cached read was MOV where an uncached one was LDR, one
 * instruction either way. This pays nothing at all: it declines to emit an
 * instruction whose result is already in place.
 *
 * The discipline is one-writer, many-destroyers. `g_r1_next` is set in
 * exactly one place -- emit_st_greg, which has just proved R1 and the
 * guest register hold the same value -- and cleared by every emitter that
 * can write R1, by emit_helper_call (r0-r3 are clobbered), at block entry,
 * and on the rollback of an untranslatable instruction. Erring towards
 * forgetting costs a missed elision; erring the other way is a wrong
 * answer, so the destroyer list is deliberately over-inclusive.
 *
 * It is consumed exactly once, by the first emit_ld_greg(R1, ...) of the
 * following instruction, which clears it immediately -- so nothing that
 * clobbers R1 later in that instruction can make it stale.
 */
static int g_r1_greg;    /* valid now, within the current instruction   */
static int g_r1_next;    /* what the next instruction may assume        */

/*
 * Where the last guest-register store landed, so it can be taken back.
 *
 * When the next instruction overwrites that register without being able to
 * trap on the way, the store was dead: nothing could have observed it. The
 * store is physically the last halfword in the code buffer at the moment
 * that becomes known, so undoing it is a decrement of the bump pointer --
 * no memmove, no fixups, and no forward reference can have been emitted
 * over it yet, which is what makes this cheap enough to be worth doing.
 *
 * `g_st_pos` is the emit position immediately after the store; comparing
 * it against the current one is how we know nothing else has been emitted
 * since.
 */
static uint16_t *g_st_pos;
static int       g_st_greg = -1;

static void jit_r1_forget(void)
{
    g_r1_greg = -1;
    g_r1_next = -1;
    g_st_pos = NULL;
...
{
    if (RV_JIT_ELIDE_LD && rt == R1) {
        if (g_r1_greg == (int)greg) {
            /* Already there: the previous instruction stored it from R1
             * and nothing has clobbered it since. */
            g_r1_greg = -1;
            g_stats.ld_elided++;
            return;
        }
        /* Whatever this instruction does to R1 next, the residency fact
...
static void emit_st_greg(uint32_t greg, uint32_t rt)
{
    ...
    /* The one place residency is established: R1 and the guest register
     * now hold the same value, by construction. */
    if (rt == R1) {
        g_r1_next = (int)greg;
        g_st_pos = g_emit;      /* just past the store, for undoing it */
        g_st_greg = (int)greg;
    } else {
        g_r1_next = -1;
        g_st_pos = NULL;
...
/*
 * Would `insn` make the previous instruction's store dead?
 *
 * Three things all have to hold, and each one is load-bearing:
 *
 *   1. `insn` overwrites the very register that was stored, so the stored
 *      value is never read from memory again;
 *   2. it reads that register as rs1, so the value it needs is the one
 *      still sitting in R1 -- which is exactly the load the residency
 *      tracker elides, so the pair costs neither the store nor the reload;
 *   3. it cannot trap before it writes. Guest state has to be coherent at
 *      every instruction boundary, because a trap unwinds to the
 *      interpreter with the hart struct as the only record of where things
 *      got to. Only a register-to-register ALU operation qualifies:
 *      anything touching memory can fault first and would then expose the
 *      register we declined to write.
 *
 * The accepted set is deliberately narrower than "cannot trap": it is the
 * encodings that also cannot *decline to translate*, since a store removed
 * for an instruction that then rolls out of the block would leave the
 * register stale. The rollback path restores it anyway, so this is a
 * second line rather than the only one -- but the cheap check is worth
 * having where the expensive consequence is a wrong answer.
 *
 * ADDI/XORI/ORI/ANDI and ADD/XOR/OR/AND are what the measured dead pairs
 * are actually made of (RV32_PAIR_STATS: addi->andi and andi->add are the
 * two largest), so a wider list would buy little.
 */
static bool kill_dead_store(uint32_t insn)
{
    if (!RV_JIT_ELIDE_ST) {
        return false;
    }
    if (g_st_greg <= 0 || g_st_pos != g_emit) {
        return false;   /* nothing pending, or something came after it */
    }
    ...
    const bool alu_imm = (op == OP_IMM) &&
                         (f3 == 0u || f3 == 4u || f3 == 6u || f3 == 7u);
    const bool alu_reg = (op == OP_OP) && (f7 == 0u) &&
                         (f3 == 0u || f3 == 4u || f3 == 6u || f3 == 7u);
    if (!alu_imm && !alu_reg) {
        return false;
    }

    if (rd != (uint32_t)g_st_greg || rs1 != (uint32_t)g_st_greg) {
        return false;
    }
    /* A reg-reg form reading it as rs2 as well needs a second copy, and
     * that one does come from memory. */
    if (alu_reg && rs2 == (uint32_t)g_st_greg) {
        return false;
    }
 ...
 */
/* Helper calls clobber r0-r3, so no residency survives one. */
 ...
 */
static bool pt_region_ok(const emu_region_t *r)
{
    return r->kind == EMU_MEM_PASSTHRU && r->size != 0u &&
           r->host_base == (uintptr_t)r->base &&
           (r->perm & EMU_PERM_R) != 0u && r->widths == EMU_WANY;
}

static void scan_passthru(const emu_bus_t *bus)
{
    g_pt_base = 0u;
    g_pt_size = 0u;
    g_pt_holes = 0u;
    ...
    const emu_region_t *first = NULL;
    for (uint32_t i = 0; i < bus->count; i++) {
        const emu_region_t *r = &bus->regions[i];
...
static uint32_t translate_once(rv_hart_t *h, uint32_t pc)
{
    ...
    if (!g_regions_scanned) {
        ...
        for (uint32_t i = 0; i < h->bus->count; i++) {
            const emu_region_t *r = &h->bus->regions[i];
    ...
    /*
     * Nothing is resident at a block entry, and that has to stay true on
     * every iteration of a chained loop -- the back edge branches to the
     * chain target below, so code emitted there may not assume anything a
     * previous instruction left in R1.
     */
    jit_r1_forget();
    ...
    while (count < RV_JIT_MAX_BLOCK_INSNS && !emu_jit_overflowed()) {
...
#if RV_EXT_SV32
        if (h->vm_active &&
            rv_mmu_translate(h, cur, EMU_ACC_FETCH, &fcur) != RV_EXC_NONE) {
         ...
         */
        if (h->pmp_active && !rv_pmp_check(h, fcur, 2u, EMU_ACC_FETCH)) {
            break;
        }
#endif
        uint16_t lo;
        if (emu_bus_fetch16(h->bus, fcur, &lo) != EMU_FAULT_NONE) {
            break;
        }
        ...
        if (rv_is_32bit(lo)) {
            ...
            if (h->vm_active &&
                ...
                rv_mmu_translate(h, cur + 2u, EMU_ACC_FETCH, &fcur2)
                    != RV_EXC_NONE) {
                break;
            }
#endif
            ...
            if (h->pmp_active && !rv_pmp_check(h, fcur2, 2u, EMU_ACC_FETCH)) {
                break;
            }
#endif
            if (emu_bus_fetch16(h->bus, fcur2, &hi) != EMU_FAULT_NONE) {
                break;
            }
        ...
        /*
         * Carry residency across the instruction boundary. This is the
         * only place it propagates: within an instruction the fact is
         * consumed by the first load and destroyed by anything that
         * writes R1.
         */
        g_r1_greg = g_r1_next;
        g_r1_next = -1;
         ...
         * rolled back out of the block cleanly -- taken *before* the dead
         * store below is removed, so the rollback puts that back too.
         */
        uint16_t *const before = g_emit;

        /*
         * Take back the previous instruction's store if this one makes it
         * dead. kill_dead_store only accepts an instruction that cannot
         * trap before it writes, so nothing could have observed the value;
         * and if translate_one declines anyway, the rollback below puts
         * the halfword back verbatim.
         */
        const bool killed = kill_dead_store(insn);
        const uint16_t saved_st = killed ? *g_emit : 0u;
        ...
        if (r < 0) {
            /* Rolled out of the block: whatever it emitted never runs, so
             * anything it claimed about R1 must go with it -- including a
             * store this instruction talked us out of emitting. */
            if (killed) {
                before[-1] = saved_st;
            }
...
static void jit_note_frm(const rv_hart_t *h)
{
    ...
    if (EMU_UNLIKELY(fs_off != g_fs_off_seen)) {
        /*
