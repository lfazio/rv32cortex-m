...
static rv_exc_t fault_for(rv_access_t acc)
{
    switch (acc) {
    case RV_ACC_FETCH: return RV_EXC_INSN_PAGE_FAULT;
    case RV_ACC_LOAD:  return RV_EXC_LOAD_PAGE_FAULT;
 ...
 */
static uint32_t acc_priv(const rv_hart_t *h, rv_access_t acc)
{
    return (acc == RV_ACC_FETCH) ? (uint32_t)h->priv : rv_hart_data_priv(h);
}
 ...
 */
static bool leaf_permits(const rv_hart_t *h, uint32_t pte, rv_access_t acc,
                         uint32_t priv)
{
    ...
    if (priv == RV_PRIV_U) {
        ...
        if ((pte & PTE_U) != 0u) {
            if (acc == RV_ACC_FETCH || (h->mstatus & MSTATUS_SUM) == 0u) {
                return false;
            }
        }
    }
    ...
    if (acc == RV_ACC_STORE && (pte & PTE_D) == 0u) {
        return false;
    }
    return true;
}
...
static rv_exc_t access_fault_for(rv_access_t acc)
{
    switch (acc) {
    case RV_ACC_FETCH: return RV_EXC_INSN_ACCESS_FAULT;
    case RV_ACC_LOAD:  return RV_EXC_LOAD_ACCESS_FAULT;
 ...
 * their own cause.
 */
static rv_exc_t read_pte(rv_hart_t *h, uint32_t pa, rv_access_t acc,
                         uint32_t *out)
{
#if RV_EXT_PMP
    /*
    ...
    if (RV_UNLIKELY(h->pmp_active) &&
        !rv_pmp_check(h, pa, 4u, RV_ACC_LOAD)) {
        return access_fault_for(acc);
    }
#endif
    if (rv_bus_read(h->bus, pa, 4u, out) != RV_EXC_NONE) {
        return access_fault_for(acc);
    }
    return RV_EXC_NONE;
}
                          ...
                          uint32_t *pa)
{
    ...
    if (RV_LIKELY(h->tlb[slot].valid && h->tlb[slot].vpn == vpn)) {
        if (RV_LIKELY(leaf_permits(h, h->tlb[slot].pte, acc, priv))) {
