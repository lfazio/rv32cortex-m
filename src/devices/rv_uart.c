...
/*
 * rv_uart.c - NS16550-subset console UART.
 *
...
static int uart_peek(rv_uart_t *u)
{
...
static rv_exc_t uart_read(void *ctx, uint32_t off, uint32_t size, uint32_t *out)
{
    rv_uart_t *u = (rv_uart_t *)ctx;
    ...
    case RV_UART_IER: *out = u->ier; break;
    case RV_UART_IIR_FCR: *out = 0x01u; break;   /* no interrupt pending */
    case RV_UART_LCR: *out = u->lcr; break;
    case RV_UART_MCR: *out = u->mcr; break;

    case RV_UART_LSR: {
        /* TX is synchronous, so THR is permanently empty. */
        uint32_t lsr = RV_UART_LSR_THRE | RV_UART_LSR_TEMT;
        if (uart_peek(u) >= 0) {
            lsr |= RV_UART_LSR_DR;
        }
        *out = lsr;
    ...
    case RV_UART_MSR: *out = 0u; break;
    case RV_UART_SCR: *out = u->scr; break;
...
static rv_exc_t uart_write(void *ctx, uint32_t off, uint32_t size, uint32_t val)
{
    rv_uart_t *u = (rv_uart_t *)ctx;
    ...
    case RV_UART_IER: u->ier = (uint8_t)val; break;
    case RV_UART_LCR: u->lcr = (uint8_t)val; break;
    case RV_UART_MCR: u->mcr = (uint8_t)val; break;
    case RV_UART_SCR: u->scr = (uint8_t)val; break;
...
const rv_dev_ops_t rv_uart_ops = {
    .read  = uart_read,
    .write = uart_write,
    .tick  = NULL,
};
