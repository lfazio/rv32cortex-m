 ...
 * core.
 */
static EMU_ALWAYS_INLINE void wr(rv_hart_t *h, uint32_t rd, uint32_t v)
{
    h->x[rd] = v;
    h->x[0] = 0u;
}
...
#if RV_EXT_M
static EMU_ALWAYS_INLINE uint32_t mulh_ss(int32_t a, int32_t b)
{
    return (uint32_t)(((int64_t)a * (int64_t)b) >> 32);
}

static EMU_ALWAYS_INLINE uint32_t mulh_su(int32_t a, uint32_t b)
{
    /* b is zero-extended before the multiply, so the product fits in 64 bits. */
    return (uint32_t)(((int64_t)a * (int64_t)(uint64_t)b) >> 32);
}

static EMU_ALWAYS_INLINE uint32_t mulh_uu(uint32_t a, uint32_t b)
{
...
static EMU_ALWAYS_INLINE uint32_t zbb_clz(uint32_t v)
{
    return (v == 0u) ? 32u : (uint32_t)__builtin_clz(v);
}

static EMU_ALWAYS_INLINE uint32_t zbb_ctz(uint32_t v)
{
    return (v == 0u) ? 32u : (uint32_t)__builtin_ctz(v);
}

/* Rotates by a multiple of 32 must not shift by 32, which is UB in C. */
static EMU_ALWAYS_INLINE uint32_t zbb_ror(uint32_t v, uint32_t n)
{
    n &= 31u;
    return (n == 0u) ? v : ((v >> n) | (v << (32u - n)));
}

static EMU_ALWAYS_INLINE uint32_t zbb_rol(uint32_t v, uint32_t n)
{
    n &= 31u;
    return (n == 0u) ? v : ((v << n) | (v >> (32u - n)));
}
...
static EMU_ALWAYS_INLINE uint32_t zbb_orcb(uint32_t v)
{
    uint32_t r = 0u;
...
#define CTR_INSTRET 2u
        ...
        unsigned len;
        {
            if (EMU_UNLIKELY((pc & TARGET_ALIGN_MASK) != 0u)) {
            ...
            if (EMU_UNLIKELY(h->fetch_guard)) {
                 ...
                 */
                if (h->trig_active && rv_trig_check(h, pc, EMU_ACC_FETCH)) {
                ...
                if (h->vm_active) {
                    const rv_exc_t t =
                        rv_mmu_translate(h, pc, EMU_ACC_FETCH, &fpc);
            ...
            uint16_t lo;
            emu_fault_t f = emu_bus_fetch16(h->bus, fpc, &lo);
            if (EMU_UNLIKELY(f != EMU_FAULT_NONE)) {
                TRAP(rv_exc_from_fault(f), pc);
            }
            ...
            if (rv_is_32bit(lo)) {
                 ...
                 */
                if (EMU_UNLIKELY(h->pmp_active && (pc & 2u) != 0u) &&
                    !rv_pmp_check(h, fpc2, 2u, EMU_ACC_FETCH)) {
                    TRAP(RV_EXC_INSN_ACCESS_FAULT, pc + 2u);
                }
#endif
                f = emu_bus_fetch16(h->bus, fpc2, &hi);
                if (EMU_UNLIKELY(f != EMU_FAULT_NONE)) {
                    TRAP(rv_exc_from_fault(f), pc + 2u);
                }
        ...
        case OP_JAL >> 2: {
            ...
            if (EMU_UNLIKELY((target & TARGET_ALIGN_MASK) != 0u)) {
        ...
        case OP_JALR >> 2: {
            if (EMU_UNLIKELY(rv_funct3(insn) != 0u)) {
        ...
        case OP_MISC_MEM >> 2: {
            ...
            if (f3 == 2u) {
                ...
                if (EMU_UNLIKELY(rv_rd(insn) != 0u || !rv_cbo_valid(op))) {
                    TRAP(RV_EXC_ILLEGAL_INSN, insn);
                }
        ...
        case OP_SYSTEM >> 2: {
            ...
            if (f3 == 0u) {
                ...
                if (EMU_UNLIKELY(rv_rd(insn) != 0u || rv_rs1(insn) != 0u)) {
                    TRAP(RV_EXC_ILLEGAL_INSN, insn);
                }
                ...
                case 0x000u:                    /* ECALL */
                    ...
                    if (h->ecall != NULL) {
                         ...
                         * is the only place in the frontend that knows it:
                         * a7 selects, a0-a3 are the arguments, a0 takes
                         * the result.
                         */
                        emu_syscall_t sc = {
                            .nr  = h->x[17],
...
/*
 * Adapters onto emu_backend_t, which deals in emu_cpu_t because a platform
 * can hold any frontend's core. The cast back is paid once per budget --
 * 4096 instructions by default -- so it does not appear in any profile;
 * everything above stays typed as rv_hart_t.
 */
static void interp_reset_cpu(emu_cpu_t *cpu)
{
    interp_reset((rv_hart_t *)cpu);
}

static emu_run_reason_t interp_run_cpu(emu_cpu_t *cpu, uint32_t budget,
                                       uint32_t *retired)
{
    return interp_run((rv_hart_t *)cpu, budget, retired);
}
...
const emu_backend_t *rv_backend = &rv_backend_interp;

emu_run_reason_t rv_step(rv_hart_t *h)
{
    return rv_backend->run((emu_cpu_t *)h, 1u, NULL);
}
