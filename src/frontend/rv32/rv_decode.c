...
static uint32_t enc_i(uint32_t imm, uint32_t rs1, uint32_t f3, uint32_t rd,
                      uint32_t op)
{
...
static uint32_t enc_s(uint32_t imm, uint32_t rs2, uint32_t rs1, uint32_t f3,
                      uint32_t op)
{
    return ((imm & 0xFE0u) << 20) | (rs2 << 20) | (rs1 << 15) | (f3 << 12) |
           ((imm & 0x1Fu) << 7) | op;
}
...
static uint32_t enc_b(uint32_t imm, uint32_t rs2, uint32_t rs1, uint32_t f3,
                      uint32_t op)
{
    return ((imm & 0x1000u) << 19) | ((imm & 0x7E0u) << 20) | (rs2 << 20) |
           (rs1 << 15) | (f3 << 12) | ((imm & 0x1Eu) << 7) |
           ((imm & 0x800u) >> 4) | op;
}
...
static EMU_ALWAYS_INLINE uint32_t c_rd(uint16_t c)
{
    return (c >> 7) & 0x1Fu;
}
static EMU_ALWAYS_INLINE uint32_t c_rs2(uint16_t c)
{
    return (c >> 2) & 0x1Fu;
}
/* 3-bit register fields select x8..x15. */
static EMU_ALWAYS_INLINE uint32_t c_rdp(uint16_t c)
{
    return ((c >> 7) & 0x7u) + 8u;
}
static EMU_ALWAYS_INLINE uint32_t c_rs2p(uint16_t c)
{
    return ((c >> 2) & 0x7u) + 8u;
}
...
static uint32_t expand_q0(uint16_t c)
{
    ...
    case 1: { /* C.FLD -> fld rd', off(rs1') */
        uint32_t off =
            (((uint32_t)c >> 7) & 0x38u) | (((uint32_t)c << 1) & 0xC0u);
...
static uint32_t expand_q1(uint16_t c)
{
    ...
    const uint32_t ci6 =
        (((uint32_t)c >> 7) & 0x20u) | (((uint32_t)c >> 2) & 0x1Fu);

    switch (f3) {
    case 0: /* C.ADDI -> addi rd, rd, imm (rd==0 is C.NOP) */
        return enc_i((uint32_t)emu_sext(ci6, 6), c_rd(c), 0u, c_rd(c), OP_IMM);
    ...
    case 1: { /* C.JAL -> jal x1, off  (RV32 only) */
        ...
        return enc_j((uint32_t)emu_sext(off, 12), 1u, OP_JAL);
    }
    ...
    case 2: /* C.LI -> addi rd, x0, imm.  rd==0 is a HINT and expands to a
        ...
        return enc_i((uint32_t)emu_sext(ci6, 6), 0u, 0u, c_rd(c), OP_IMM);
    ...
    case 3: {
        if (c_rd(c) == 2u) { /* C.ADDI16SP -> addi x2, x2, nzimm */
            ...
            return enc_i((uint32_t)emu_sext(imm, 10), 2u, 0u, 2u, OP_IMM);
        }
        ...
        return enc_u((uint32_t)emu_sext(ci6, 6) & 0xFFFFFu, c_rd(c), OP_LUI);
    }
    ...
    case 4: { /* MISC-ALU */
        ...
        if (sel == 0u || sel == 1u) { /* C.SRLI / C.SRAI */
            ...
            return enc_i((sel == 1u ? 0x400u : 0u) | shamt, c_rdp(c), 5u,
                         c_rdp(c), OP_IMM);
        }
        if (sel == 2u) { /* C.ANDI */
            return enc_i((uint32_t)emu_sext(shamt, 6), c_rdp(c), 7u, c_rdp(c),
                         OP_IMM);
        }
        ...
        switch ((c >> 5) & 0x3u) {
        case 0:
            return enc_r(0x20u, c_rs2p(c), c_rdp(c), 0u, c_rdp(c),
                         OP_OP); /* SUB */
        case 1:
            return enc_r(0x00u, c_rs2p(c), c_rdp(c), 4u, c_rdp(c),
                         OP_OP); /* XOR */
        case 2:
            return enc_r(0x00u, c_rs2p(c), c_rdp(c), 6u, c_rdp(c),
                         OP_OP); /* OR  */
        default:
            return enc_r(0x00u, c_rs2p(c), c_rdp(c), 7u, c_rdp(c),
                         OP_OP); /* AND */
        }
    }
    ...
    case 5: { /* C.J -> jal x0, off */
        uint32_t off =
            (((uint32_t)c >> 1) & 0x800u) | (((uint32_t)c >> 7) & 0x10u) |
            (((uint32_t)c >> 1) & 0x300u) | (((uint32_t)c << 2) & 0x400u) |
            (((uint32_t)c >> 1) & 0x40u) | (((uint32_t)c << 1) & 0x80u) |
            (((uint32_t)c >> 2) & 0xEu) | (((uint32_t)c << 3) & 0x20u);
        return enc_j((uint32_t)emu_sext(off, 12), 0u, OP_JAL);
    }
    ...
    case 7: { /* C.BNEZ */
        uint32_t off = (((uint32_t)c >> 4) & 0x100u) /* off[8]   */
                       ...
                       | (((uint32_t)c << 3) & 0x20u); /* off[5]   */
        return enc_b((uint32_t)emu_sext(off, 9), 0u, c_rdp(c),
...
static uint32_t expand_q2(uint16_t c)
{
    ...
    case 7: { /* C.FSWSP -> fsw rs2, off(x2)  (Zcf) */
        uint32_t off =
            (((uint32_t)c >> 7) & 0x3Cu) | (((uint32_t)c >> 1) & 0xC0u);
    ...
    case 5: { /* C.FSDSP -> fsd rs2, off(x2)  (Zcd) */
        uint32_t off =
            (((uint32_t)c >> 7) & 0x38u) | (((uint32_t)c >> 1) & 0x1C0u);
...
uint32_t rv_decode_expand_c(uint16_t c)
{
    ...
    case 2:
        return expand_q2(c);
    default:
        return ILLEGAL; /* 0b11 is not a compressed instruction */
    }
}
