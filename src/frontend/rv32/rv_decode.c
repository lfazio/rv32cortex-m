...
static EMU_ALWAYS_INLINE uint32_t c_rd(uint16_t c)   { return (c >> 7) & 0x1Fu; }
static EMU_ALWAYS_INLINE uint32_t c_rs2(uint16_t c)  { return (c >> 2) & 0x1Fu; }
/* 3-bit register fields select x8..x15. */
static EMU_ALWAYS_INLINE uint32_t c_rdp(uint16_t c)  { return ((c >> 7) & 0x7u) + 8u; }
static EMU_ALWAYS_INLINE uint32_t c_rs2p(uint16_t c) { return ((c >> 2) & 0x7u) + 8u; }
...
static uint32_t expand_q1(uint16_t c)
{
    ...
    case 4: { /* MISC-ALU */
        ...
        if (sel == 2u) {                        /* C.ANDI */
            return enc_i((uint32_t)emu_sext(shamt, 6), c_rdp(c), 7u,
                         c_rdp(c), OP_IMM);
        }
