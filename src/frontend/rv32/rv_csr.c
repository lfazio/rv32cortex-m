...
static EMU_ALWAYS_INLINE bool csr_is_readonly(uint32_t csr)
{
    return ((csr >> 10) & 0x3u) == 0x3u;
}

/* csr[9:8] is the lowest privilege level allowed to access it. */
static EMU_ALWAYS_INLINE uint32_t csr_min_priv(uint32_t csr)
{
    return (csr >> 8) & 0x3u;
}
...
{
    if (EMU_UNLIKELY(csr_min_priv(csr) > h->priv)) {
...
{
    if (EMU_UNLIKELY(csr_min_priv(csr) > h->priv || csr_is_readonly(csr))) {
        return RV_EXC_ILLEGAL_INSN;
    }
