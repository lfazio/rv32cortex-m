...
void SystemInit(void)
{
  ...
  /*
   * ---- rv32cortex-m: bring up AXISRAM3..6 --------------------------
   *
   * The boot ROM leaves these four banks powered down, and the symptom
   * is not an error: a firmware whose .bss or stack lands there simply
   * goes silent, and reading 0x34200000 through a debugger fails
   * outright. That is 1792 KB of the part's 3.8 MB unreachable by
   * default.
   *
   * The sequence is ST's own, from STM32CubeN6's
   * Projects/STM32N6570-DK/Applications/VENC/VENC_USB FSBL: ungate the
   * clocks through MEMENSR -- the *set* alias, so this is one write and
   * not a read-modify-write -- then clear each bank's shutdown bit in
   * its RAMCFG instance.
   *
   * **It has to happen here rather than in board_hw_init.** SystemInit
   * runs before the startup code copies .data and zeroes .bss, and both
   * of those now target this memory; doing it later would mean the copy
   * had already written into banks that were not answering.
   *
   * This file is otherwise the vendor's Templates/system_stm32n6xx_fsbl.c
   * unchanged. It is copied into the tree rather than patched at build
   * time because a local copy is greppable and a sed in CMake is not.
   */
