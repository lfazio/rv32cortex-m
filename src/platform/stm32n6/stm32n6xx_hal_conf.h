/**
  ...
  *
  * Started from the vendor's stm32n6xx_hal_conf_template.h and trimmed to
  * the modules this firmware links. **Not renamed from another family's**:
  * CLAUDE.md records the F4-to-F7 move, where a renamed F4 config left the
  * ART accelerator off because the two families spell the option
  * differently -- INSTRUCTION_CACHE_ENABLE against ART_ACCELERATOR_ENABLE
  * -- and nothing said so.
...
/*#define HAL_CACHEAXI_MODULE_ENABLED */
/*
 * What this firmware actually uses, enabled here rather than by
 * uncommenting the template's list above -- so that a diff against
 * stm32n6xx_hal_conf_template.h shows exactly what was chosen.
 *
 * It is a short list because the guest owns the peripherals: everything
 * beyond bringing the part up and opening a console belongs in a guest
 * driver reaching real registers through the passthrough window. Adding
 * a HAL module here is almost always the wrong fix.
 *
 * **Do not rename an F4 or F7 conf file into this.** The families spell
 * things differently in ways that compile: F4 has
 * INSTRUCTION_CACHE_ENABLE / DATA_CACHE_ENABLE where F7 has
 * ART_ACCELERATOR_ENABLE, and a renamed file leaves the accelerator off
 * with nothing to show for it. This one started from the N6 family's own
 * template.
 */
