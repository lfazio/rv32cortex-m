...
{
    start_time_val = now_us();
}
void stop_time(void)
{
    stop_time_val = now_us();
}
...
static void emit_num(ee_u32 v, unsigned base, int is_signed, int width,
                     int zero_pad, int upper)
{
    char tmp[12];
    int n = 0, neg = 0;
    ...
    do {
        ...
        tmp[n++] =
            (char)(d < 10u ? ('0' + d) : ((upper ? 'A' : 'a') + d - 10u));
        v /= base;
    } while (v != 0u);
    if (neg) {
        tmp[n++] = '-';
...
int ee_printf(const char *fmt, ...)
{
    ...
    for (const char *p = fmt; *p != '\0'; p++) {
        ...
        case 'd':
        case 'i':
            emit_num((ee_u32) __builtin_va_arg(ap, int), 10u, 1, width,
                     zero_pad, 0);
            break;
        case 'u':
