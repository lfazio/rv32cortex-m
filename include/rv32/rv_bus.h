...
/*
 * rv_bus.h - Guest physical address space.
 *
 ...
 * Four region kinds:
 *
 *   RV_MEM_RAM       backed by a host buffer, read/write
 *   RV_MEM_ROM       backed by a host buffer, writes fault
 *   RV_MEM_MMIO      handled by a device callback (virtual devices)
 *   RV_MEM_PASSTHRU  translated onto real ARM addresses and accessed
 *                    directly, subject to the region's perm/width mask
 *
 * PASSTHRU is what lets the RISC-V guest drive the STM32's own peripherals.
 ...
 */
#ifndef RV32_RV_BUS_H
#define RV32_RV_BUS_H
...
/* Access permissions. */
#define RV_PERM_R   0x1u
#define RV_PERM_W   0x2u
#define RV_PERM_X   0x4u
#define RV_PERM_RW  (RV_PERM_R | RV_PERM_W)
#define RV_PERM_RX  (RV_PERM_R | RV_PERM_X)
#define RV_PERM_RWX (RV_PERM_R | RV_PERM_W | RV_PERM_X)

/* Permitted access widths. Some ARM peripherals fault on the wrong width. */
#define RV_W8       0x1u
#define RV_W16      0x2u
#define RV_W32      0x4u
#define RV_WANY     (RV_W8 | RV_W16 | RV_W32)

/* Access kind, used to pick the right fault cause. */
typedef enum {
    RV_ACC_FETCH = 0,
    RV_ACC_LOAD,
    RV_ACC_STORE,
...
typedef struct rv_dev_ops {
    /* off is the byte offset within the region; size is 1, 2 or 4. */
    rv_exc_t (*read)(void *ctx, uint32_t off, uint32_t size, uint32_t *out);
    rv_exc_t (*write)(void *ctx, uint32_t off, uint32_t size, uint32_t val);
    /* Optional: advance device time / raise interrupts. May be NULL. */
    void     (*tick)(void *ctx, struct rv_hart *hart);
} rv_dev_ops_t;

typedef struct rv_region {
    ...
    const rv_dev_ops_t *ops;   /* MMIO only                             */
    void       *ctx;           /* MMIO only                             */
    const char *name;
    uint8_t     kind;      /* rv_region_kind_t                          */
    uint8_t     perm;      /* RV_PERM_*                                 */
    uint8_t     widths;    /* RV_W*                                     */
    uint8_t     flags;
} rv_region_t;
...
typedef struct rv_bus {
    ...
    rv_region_t regions[RV_MAX_REGIONS];
    uint32_t    count;
 ...
 */
rv_exc_t rv_bus_read_slow(rv_bus_t *bus, uint32_t addr, uint32_t size, uint32_t *out);
rv_exc_t rv_bus_write_slow(rv_bus_t *bus, uint32_t addr, uint32_t size, uint32_t val);
 ...
 * naturally aligned (rv_hart_load/rv_hart_store enforce that so the fault
 * can be reported with the right cause). On success returns RV_EXC_NONE
 * and stores to *out; on failure returns the cause to report and leaves
 * *out untouched. Loads are zero-extended; sign extension is the caller's
 * job.
 */
static RV_ALWAYS_INLINE rv_exc_t rv_bus_read(rv_bus_t *bus, uint32_t addr,
                                             uint32_t size, uint32_t *out)
{
    ...
    if (RV_LIKELY(off < bus->data_span && (bus->data_span - off) >= size)) {
    ...
    return rv_bus_read_slow(bus, addr, size, out);
}

static RV_ALWAYS_INLINE rv_exc_t rv_bus_write(rv_bus_t *bus, uint32_t addr,
                                              uint32_t size, uint32_t val)
{
    ...
    return rv_bus_write_slow(bus, addr, size, val);
}

/* Instruction fetch of a 16-bit parcel. Requires RV_PERM_X. */
static RV_ALWAYS_INLINE rv_exc_t rv_bus_fetch16(rv_bus_t *bus, uint32_t addr,
                                                uint16_t *out)
{
    const uint32_t off = addr - bus->fetch_base;
    if (RV_LIKELY(off < bus->fetch_span)) {
        *out = *(const uint16_t *)(const void *)(bus->fetch_host + off);
        return RV_EXC_NONE;
    }
    return rv_bus_fetch16_slow(bus, addr, out);
}
